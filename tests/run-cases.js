...
const syllabusAssertions = require("../data/syllabus-assertions.json");
const {
...
assert.equal(JULY_ARCADE_GAME_ASSERTIONS.length, 6);
assert.ok(JULY_ARCADE_GAME_ASSERTIONS.every((game) => game.url.includes(`games/${game.id}`)));
...
const targetPlan = getNextTargetPlan(sampleProgress, scrapedSample.arcade_games_completed, scrapedSample.skill_badges_completed, scrapedSample);
